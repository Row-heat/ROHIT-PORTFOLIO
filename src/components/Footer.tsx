...
const Footer: React.FC = () => {
  ...
  return (
    <footer ref={footerRef} className="relative py-12 px-6 bg-slate-900/80 backdrop-blur-sm border-t border-white/10">
      {/* Floating particles */}
      <div className="absolute inset-0 overflow-hidden">
        {[...Array(15)].map((_, i) => (
          <div
            key={i}
            className="footer-particle absolute w-1 h-1 bg-cyan-400/20 rounded-full"
            style={{
          ...
          <nav className="flex gap-8">
            {navItems.map((item) => (
              <a
                ...
                className="text-white/60 hover:text-cyan-400 transition-colors duration-300 text-sm tracking-wide"
              >
                {item}
              </a>
            ))}
